...
// کلید قدرتمند API اکسپلوررها
const SCAN_API_KEY = "GAEBTXE3Z1NTZAJCDNAGC6YQEMZIAYZVFW";

// آدرس‌های رسمی قرارداد تتر
const TRON_USDT_CONTRACT = "TR7NHqjeKQxGTCi8q8ZY4pL8otSzgjLj6t";
const ETH_USDT_CONTRACT = "0xdac17f958d2ee523a2206206994597c13d831ec7";
const BSC_USDT_CONTRACT = "0x55d398326f99059ff775485246999027b3197955";
const AVAX_USDT_CONTRACT = "0x9702230A8Ea53601f5cD2dc00fDBc13d4dF4A8c7"; // قرارداد رسمی USDT اولانچ

// شبکه‌های EVM - مجهز به API اکسپلورر و RPCهای پایدار عمومی
const EVM_NETWORKS = {
    eth: { name: "Ethereum", rpc: "https://ethereum-rpc.publicnode.com", explorer_api: "https://api.etherscan.io/api", api_key: SCAN_API_KEY, symbol: "ETH", usdt: ETH_USDT_CONTRACT },
    bsc: { name: "BSC", rpc: "https://bsc-rpc.publicnode.com", explorer_api: "https://api.bscscan.com/api", api_key: SCAN_API_KEY, symbol: "BNB", usdt: BSC_USDT_CONTRACT },
    polygon: { name: "Polygon", rpc: "https://polygon-rpc.com", symbol: "MATIC", usdt: null },
    avalanche: { name: "Avalanche", rpc: "https://api.avax.network/ext/bc/C/rpc", symbol: "AVAX", usdt: AVAX_USDT_CONTRACT }
};
...
// تنظیمات CORS و Header برای دسترسی آزاد از اینترنت
...
// اصلاح ساختار موتور دوگانه برای جلوگیری از خطای همپوشانی و لیمیت شبکه‌ها
async function evmRpcCall(net, method, params) {
    if (net.explorer_api && net.api_key) {
        try {
            let url = `${net.explorer_api}?module=proxy&action=${method}&apikey=${net.api_key}`;
            if (method === 'eth_getTransactionByHash' || method === 'eth_getTransactionReceipt') {
                url += `&txhash=${params[0]}`;
            } else if (method === 'eth_call') {
                url += `&to=${params[0].to}&data=${params[0].data}&tag=${params[1] || 'latest'}`;
            } else if (method === 'eth_getBalance' || method === 'eth_getCode') {
                url += `&address=${params[0]}&tag=${params[1] || 'latest'}`;
            }
            const res = await makeHttpRequest(url, 'GET');

            // تایید صحت ساختاری پاسخ دریافتی از اکسپلورر پیش از بازگشت نهایی مدار
            if (res && (res.jsonrpc === "2.0" || (res.result && typeof res.result === 'object') || (method === 'eth_getCode' && res.result))) {
                return res;
            }
        } catch (e) {
            console.error(`[EXPLORER EXCEPTION] ${net.name}:`, e.message);
        }
    }

    // بازگشت خودکار روی لایه نود بومی در صورت لیمیت یا در دسترس نبودن فیلدها
    return await makeHttpRequest(net.rpc, 'POST', { jsonrpc: "2.0", method: method, params: params, id: Date.now() });
}
...
async function queryTokenMetadataFromServer(net, contractAddress) {
    try {
        const symRes = await evmRpcCall(net, "eth_call", [{ to: contractAddress, data: "0x95d89b41" }, "latest"]);
        const decRes = await evmRpcCall(net, "eth_call", [{ to: contractAddress, data: "0x313ce567" }, "latest"]);
        const nameRes = await evmRpcCall(net, "eth_call", [{ to: contractAddress, data: "0x06fdde03" }, "latest"]);
        return {
            symbol: symRes && symRes.result && typeof symRes.result === 'string' ? decodeRpcString(symRes.result) : "TOKEN",
            name: nameRes && nameRes.result && typeof nameRes.result === 'string' ? decodeRpcString(nameRes.result) : "ERC20 Asset",
            decimals: decRes && decRes.result && typeof decRes.result === 'string' && decRes.result !== "0x" ? parseInt(decRes.result, 16) : 18
        };
    } catch { return { symbol: "TOKEN", name: "ERC20 Asset", decimals: 18 }; }
}

// استخراج داینامیک اطلاعات توکن‌های TRC20 شبکه ترون
async function queryTronTokenMetadata(contractAddress) {
    try {
        const res = await makeHttpRequest(`https://apilist.tronscanapi.com/api/token_trc20?contract=${contractAddress}`);
        if (res && res.trc20_tokens && res.trc20_tokens.length > 0) {
            const token = res.trc20_tokens[0];
            return {
                symbol: token.symbol || "TRC20",
                name: token.name || "Unknown TRC20 Asset",
                decimals: token.decimals !== undefined ? token.decimals : 6
            };
        }
    } catch(e) {}
    return { symbol: "TRC20", name: "Unknown TRC20 Token", decimals: 6 };
}
...
async function scanTxAcrossChains(txHash) {
    ...
    if (cleanHash.startsWith("0x") || /^[0-9a-fA-F]{64}$/.test(cleanHash)) {
        ...
        for (const [key, net] of Object.entries(EVM_NETWORKS)) {
            try {
                const txRes = await evmRpcCall(net, "eth_getTransactionByHash", [evmHash]);

                if (txRes && txRes.result && typeof txRes.result === 'object' && txRes.result.hash) {
                    ...
                    if (inputData !== "0x" && inputData !== "0x00") {
                        ...
                        if (methodId === "0xa9059cbb" && inputData.length >= 138) {
                            ...
                            const tokenMeta = await queryTokenMetadataFromServer(net, contractAddress);
                    ...
                    const receiptRes = await evmRpcCall(net, "eth_getTransactionReceipt", [evmHash]);
                    if (receiptRes && receiptRes.result && typeof receiptRes.result === 'object') {
        ...
        try {
            ...
            if (tronTx && tronTx.txID) {
                ...
                if (contractWrapper.type === "TriggerSmartContract") {
                    ...
                    const tronMeta = await queryTronTokenMetadata(contractAddress);
                    tokenSymbol = tronMeta.symbol;
                    tokenName = tronMeta.name;
                    decimals = tronMeta.decimals;
...
async function scanAddressAcrossChains(address) {
    ...
    if (/^0x[a-fA-F0-9]{40}$/.test(cleanAddress)) {
        for (const [key, net] of Object.entries(EVM_NETWORKS)) {
            try {
                const balRes = await evmRpcCall(net, "eth_getBalance", [cleanAddress, "latest"]);
                if (balRes && balRes.result) {
                    found = true; activeNetwork = net.name;
                    const valueHex = typeof balRes.result === 'string' && balRes.result.startsWith('0x') ? balRes.result : "0x0";
                    const value = (Number(BigInt(valueHex)) / 1e18).toFixed(6);
                    nativeBalance = `${value} ${net.symbol}`;
                    tags.push(net.name);

                    const codeRes = await evmRpcCall(net, "eth_getCode", [cleanAddress, "latest"]);
                    if (codeRes && codeRes.result && typeof codeRes.result === 'string' && codeRes.result !== "0x") addressType = "contract";
                    break;
                }
            } catch {}
        }
...
// API پابلیک برای دریافت اطلاعات رسید
...
// فقط زمانی سرور را روی پورت اجرا کن که فایل مستقیماً اجرا شده باشد (نه در حالت تست)
if (require.main === module) {
    app.listen(PORT, '0.0.0.0', () => {
        console.log(`[CORE] Server running on port ${PORT}`);
        console.log(`[CORE] Supported networks: Ethereum, BSC, Polygon, Avalanche, Bitcoin, TRON, TON`);
    });
}

// استخراج توابع و آبجکت‌ها برای استفاده در فایل‌های تست
module.exports = {
    app,
    db,
    generateShortId,
